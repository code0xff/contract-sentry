...
- lint_cmd: cd backend && ~/.local/bin/ruff check app/ tests/
- build_cmd: cd backend && ~/.local/bin/python3 -c "import app.main; print('import ok')"
- test_cmd: cd backend && DATABASE_URL=sqlite+aiosqlite:///:memory: CELERY_BROKER_URL=memory:// CELERY_RESULT_BACKEND=cache+memory:// ~/.local/bin/pytest tests/ -x -q
- security_cmd: unset
